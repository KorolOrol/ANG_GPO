...
namespace BaseClasses_V2.Model
{
    ...
    public class Element : PlotEntity, IEquatable<Element>
    {
        ...
        public Element(string hash, ElemType type, string name = "", string description = "",
            Dictionary<string, object> @params = null, int time = -1)
        {
            Hash = hash;
            _type = type;
        ...
        private string GetValueString(object value)
        {
            ...
            {
                var pairs = dict.Select(kvp => $"{kvp.Key}: {GetValueString(kvp.Value)}");
                return $"{{{string.Join(", ", pairs)}}}";
            }

            if (value is IEnumerable enumerable)
            {
                var items = enumerable.Cast<object>().Select(GetValueString);
