...
internal class Program
{
    ...
    {
        // ===== ТЕСТОВЫЙ СЦЕНАРИЙ: ДЕМОНСТРАЦИЯ КЛЮЧЕВЫХ ФУНКЦИЙ СИСТЕМЫ ПРОЦЕДУРНОЙ ГЕНЕРАЦИИ =====

        // 1. СОЗДАНИЕ ПЕРВОГО ПЕРСОНАЖА (ТОНИ) И ГЕНЕРАЦИЯ ЧЕРТ
        // Создаем персонажа "Тони Сальвоне", 25 лет, мужской пол (true = мужской)
        PrCharacter Tony = new PrCharacter("Тони", "Сальвоне", 25, true);

        // Генерируем 4 логически совместимые черты характера для Тони
        PrGenerator.CreateByLogicRandomTraits(Tony, 4);

        // 2. СОЗДАНИЕ ВТОРОГО ПЕРСОНАЖА (ПЕЙНА) С ПОЛНЫМ НАБОРОМ ХАРАКТЕРИСТИК
        // Создаем персонажа "Пейн Фуфил", 25 лет, мужской пол
        PrCharacter Pain = new PrCharacter("Пейн", "Фуфил", 25, true);

        // Генерируем 4 логически совместимые черты характера для Пейна
        PrGenerator.CreateByLogicRandomTraits(Pain, 4);

        // Генерируем 2 фобии для Пейна методом хаотичной генерации
        PrGenerator.CreateByChaoticRandomPhobias(Pain, 2);

        // Автоматически создаем текстовое описание персонажа на основе его черт и фобий
        // Метод объединяет описания всех характеристик в связный нарративный текст
        PrGenerator.CreateDesc(Pain);
        // Рассчитываем отношение Пейна к Тони на основе их черт характера
        // Используется матрица отношений для определения совместимости и уровня симпатии/антипатии
