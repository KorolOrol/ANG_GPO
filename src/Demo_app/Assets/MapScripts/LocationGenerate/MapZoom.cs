...
using System.Linq;
using MapScripts.HouseGeneration;
using UnityEngine;
...
namespace MapScripts.LocationGenerate
{
    ...
    public class MapZoom : MonoBehaviour
    {
        ...
        [FormerlySerializedAs("MBGPrefab")] public ModularBuildingGenerator mbgPrefab;

        private GameObject _buildsContainer;
        private GameObject _mapGenContainer;
        ...
        public void ReturnToMainMap()
        {
            ...
            if (_mapGenContainer) _mapGenContainer.SetActive(true);
            if (sourceRenderer) sourceRenderer.gameObject.SetActive(true);
            if (_buildsContainer) _buildsContainer.SetActive(true);

            // Удаляем контейнер 3D-города если есть
            var cityContainer = GameObject.Find("3DCityContainer");
            if (cityContainer) DestroyImmediate(cityContainer);

            if (mapDisplay) mapDisplay.gameObject.SetActive(true);

            SetChildrenVisibility(mapGenMan.gameObject, true);

            var zoomDisplay = GameObject.Find("ZoomDisplay");
            if (zoomDisplay) zoomDisplay.SetActive(false);
        }
        ...
        private static void SetChildrenVisibility(GameObject toWho, bool isVisible)
        {
        ...
        {
            // Находим контейнер со зданиями
            var buildingsContainer = GameObject.Find("BuildingsContainer")?.transform;

            if (GameObject.Find("3DCityContainer"))
            {
            ...
            var container = new GameObject("3DCityContainer");

            // Скрываем карту
            var mapGen = GameObject.Find("MapGenerator");
            if (mapGen) _mapGenContainer = mapGen; mapGen.SetActive(false);
            if (sourceRenderer) sourceRenderer.gameObject.SetActive(false);
            if (buildingsContainer)
            {
                _buildsContainer = buildingsContainer.gameObject;
            ...
            foreach (Transform building in buildingsContainer)
            {
                ...
                var builtHouse = GameObject.Find("Generated Building");
                builtHouse.name = "Generated Building " + buildingCount;
                buildingCount++;
        ...
        {
            var mapGen = FindFirstObjectByType<MapGeneratorManual>();
            if (mapGen)
            {
        ...
        private void GenerateZoomedMap()
        {
            ...
            //var manual = FindObjectsByType<MapGeneratorManual>();
            ...
            foreach ((string _, var mapPos, var bufPos) in pivots)
            {
        ...
        private void SpawnBuilding(int bx, int by, int baseSize, Transform container)
        {
            ...
            var targetRendererTransform = targetRenderer.transform;
            ...
            float wS = 10f * targetRendererTransform.localScale.x;
            var c = targetRendererTransform.position;
            var r = targetRendererTransform.right;
            var fwd = targetRendererTransform.forward;
            ...
            float cellWorldSize = 10f * targetRendererTransform.localScale.x / baseSize;
        ...
        {
            var targetRendererTransform = targetRenderer.transform;
            for (int i = targetRendererTransform.childCount - 1; i >= 0; i--)
                if (targetRendererTransform.GetChild(i).name != "BuildingsContainer")
                    DestroyImmediate(targetRendererTransform.GetChild(i).gameObject);
            ...
            foreach (var obj in GameObject.FindGameObjectsWithTag("GeneratedObject"))
            {
                ...
                var fwd = targetRendererTransform.forward;
                var wp = c + r * ((u - 0.5f) * wS) + fwd * ((v - 0.5f) * wS);
                var m = Instantiate(locationPrefab,
                    wp + targetRendererTransform.up * 0.5f,
                    obj.transform.rotation,
                    targetRendererTransform);
